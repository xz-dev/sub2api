...
// Rerank handles the OpenAI-compatible Rerank API (e.g. SiliconFlow
// reranker models) by passing the request through to the upstream as-is.
// POST /v1/rerank
func (h *OpenAIGatewayHandler) Rerank(c *gin.Context) {
	...
	// 分组利润控制：rerank 文本入口请求级装门并固定 pricingAt。
	...
	for {
		...
		if err != nil {
			if failoverClientGone(c) {
				reqLog.Info("openai_rerank.account_select_aborted_client_disconnected", zap.Error(err))
				return
			}
		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if failoverClientGone(c) {
					reqLog.Info("openai_rerank.failover_aborted_client_disconnected",
